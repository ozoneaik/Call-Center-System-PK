...
export const receiveApi = async (rateId, roomId) => {
  try {
    const { data, status } = await axiosClient.post(`${messages}/receive`, {
      rateId,
      roomId,
    });
    return { data, status };
...
//รับเรื่อง Shopee
export const receiveApiShopee = async (rateId = "", roomId = "") => {
  try {
    const { data, status } = await axiosClient.post(
      `${messages}/shopee/receive`,
      { rateId, roomId }
    );
    return { data, status };
  } catch (error) {
...
export const senToApi = async ({
  rateId,
  activeConversationId,
  latestRoomId,
}) => {
  try {
    const { data, status } = await axiosClient.post(`${messages}/sendTo`, {
      rateId,
      activeConversationId,
      latestRoomId,
    });
...
export const pauseTalkApi = async ({ activeConversationId, rateId }) => {
  try {
    const { data, status } = await axiosClient.post(`${messages}/pauseTalk`, {
      activeConversationId,
      rateId,
    });
    return { data, status };
...
export const endTalkApi = async ({
  ...
  try {
    const { data, status } = await axiosClient.post(`${messages}/endTalk`, {
      rateId,
      activeConversationId,
      tagId,
      Assessment,
...
export const endTalkAllPendingApi = async (props) => {
  ...
  const Filter = list.filter(
    (item) => item.updated_at > startTime && item.updated_at < endTime
  );
  console.log("Filter >> ", Filter);
  try {
    const { data, status } = await axiosClient.post(
      `/endTalkAllPending/${roomId}`,
      {
        list: Filter,
        startTime,
        endTime,
