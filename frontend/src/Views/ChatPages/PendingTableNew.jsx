...
export default function PendingTableNew({
  setFilterPending,
  filterPending,
  disable,
  pending,
                ...
                <td>
                  <Stack spacing={1}>
                    ...
                    <Chip color="primary" startDecorator={<MessageSharp />}>
                      {row.latest_message.contentType === "text"
                        ? row.latest_message.content
                        : "ส่งรูปภาพหรือสติกเกอร์"}
                    </Chip>
                  </Stack>
                </td>
                <td>
                ...
                <td>
                  <Stack spacing={1}>
                    <Button
                      variant="soft"
                      onClick={() =>
                        handleChat({ rateId: row.rateRef, roomId: row.roomId })
                      }
                    >
                      รับเรื่อง
                    </Button>
