...
class UcClosureStatsController extends Controller
{
    ...
    public function inProgressCases(Request $request)
    {
        ...
        $q = DB::connection('pgsql_real')->table('rates as r')
            ...
            ->leftJoin('chat_rooms as cr', 'cr.roomId', '=', 'ac.roomId')
            ->where('r.status', 'progress')
            // อิงเกณฑ์เดียวกับตัวนับ inProgressByBusinessHours: วันที่ที่ "รับเรื่อง" (ไม่ใช่ updated_at)
        ...
        // เวลาที่รอมาแล้วจนถึงตอนนี้ (elapsed)
