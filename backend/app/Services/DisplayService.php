    ...
    public function MessageList($roomId, $status)
    {
        if ($status === 'progress') {
            $data = Rates::query()->leftJoin('active_conversations', 'active_conversations.rateRef', '=', 'rates.id')
                ...
                ->select('chat_rooms.roomName', 'customers.custName', 'customers.avatar', 'customers.id AS pcust_id', 'customers.description', 'active_conversations.*', 'rates.status', 'users.name as empName', 'users.real_name as empRealName')
